...
final class Html2Text {
	...
	public function convert(string $html, dict<string, bool> $options = dict[]): string {
		$options = dict(\array_merge(static::$default_options, $options));

		// check all options are valid
		foreach ($options as $key => $value) {
			if (!\in_array($key, \array_keys(static::$default_options))) {
	 ...
	 */
	private function getDocument(string $html, bool $ignore_error = false): \DOMDocument {
	...
	private function iterateOverNode(
		mixed $node,
		?string $prevName = null,
		bool $in_pre = false,
		bool $is_office_document = false,
